...
class AddAddressModel with ChangeNotifier {
  ...
  bool _verifyInputs(String name,String address,String state,String phone,String zipCode){
  ...
  Future<void> addAddress(
    ...
    if(_verifyInputs(name, address, state, phone,zipCode)){
      ...
      (editedId == null) ? await database.setData({
        'name': name,
        'address': address,
        'state': state,
      ...
      }, 'users/$uid/addresses/$id') :await  database.updateData({
        'name': name,
        'address': address,
        'state': state,
        'country': country.name,
  ...
  AddAddressModel({@required this.database, @required this.uid}):assert(uid!=null && database!=null);
}
