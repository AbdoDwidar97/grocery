...
class PaymentModel with ChangeNotifier {
    ...
    if (paymentViaDelivery) {
      ...
      try {
        ...
        String paymentReference = await StripeService.payWithCard(
            ((Decimal.parse(order.toString()) + Decimal.parse(shippingMethod.price.toString())) * Decimal.parse('100')).toString());
        ...
        if(e is PlatformException){
          ...
          if(e.message!='cancelled'){
            showDialog(context: context,
                builder: (context)=> Dialogs.error(context, message: e.message)
            );
  ...
  Future _submitOrder(
    ...
    String id = dateTime.day.toString() + dateTime.hour.toString() +dateTime.minute.toString()+ dateTime.microsecond.toString();
    ...
    await database.setData({
      ...
      "status": "Processing",
      "order": order,
      "total": num.parse((Decimal.parse(order.toString()) + Decimal.parse(shippingMethod.price.toString())).toString()),
